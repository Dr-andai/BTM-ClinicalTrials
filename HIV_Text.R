...
KE_clinicals <- xmlToDataFrame("data/KenyaTrials.xml")
TZ_clinicals <- xmlToDataFrame("data/TanzaniaTrials.xml")
UG_clinicals <- xmlToDataFrame("data/UgandaTrials.xml")


## explore Kenya xml FROM 2023:01-2023:12

#Transform Date Column from character to Date variable
class(KE_clinicals$Date_registration)##'character'
KE_clinicals <- KE_clinicals %>%
  mutate(Date_registration = lubridate::dmy(Date_registration))

class(KE_clinicals$Date_registration)##'Date'

# change column names to lower case
names(KE_clinicals)
KE_clinicals <- janitor::clean_names(KE_clinicals)


kenya_23 <- KE_clinicals[KE_clinicals$date_registration >= "2023-01-01" & KE_clinicals$date_registration <= "2023-12-31", ]

## Extract trials with NCT ID as trial id
pattern <- "^NCT"  # Extract NCT
nct_id <- kenya_23[grep(pattern, kenya_23$trial_id), ]

#reset row names
nct_id <- nct_id[ , , drop = FALSE]  # This drops the existing row names
rownames(nct_id) <- NULL  # Set row names to NULL to reset the index
#Get row 10
nct_id[10,'countries']
...
data <- years_m[ , c('trial_id','year','month','date_registration','phase','primary_sponsor','condition','scientific_title',
                     'intervention','countries','target_size','primary_outcome','secondary_outcome','web_address')]

## Data Cleaning
# Look for missing values
pct_miss(data$condition) ## 
# drop na
...
data <- unique( data[ , c('trial_id','year','month','date_registration','phase','primary_sponsor','condition',
...
data$text <- paste(data$condition, sep=" ")
row.names(data) <- NULL
...
word_freq <- textstat_frequency(bigram, groups = data_1$year)
...
corp_dfm_cleaned <- corp_dfm[rowSums(corp_dfm) > 0, ]
